...
ggplot(df,aes(x=day))+geom_bar(fill="gold3")+ theme_minimal()+ theme_minimal()

#library(ggridges)

df %>% filter(day>1 & day<16) %>%
...
my_custom_values<-as.data.frame(custom_distrib)
ggplot(custom_distrib,aes(x=custom_distrib))+geom_histogram()
```

##le traitement des emojis.

 c'est une question essentielle mais résolue par [Sophie Balech]() .....
