...
df_nrc<- readRDS(file = "df_nrc.rds") 
...
voir le script calcul pour le détail.
...
```{r emo02c, include=TRUE, fig.height=9}
emocol<-c("red3","orangered1","purple3","royalblue3","chartreuse","olivedrab3","green4","gold") #en respectant des codes mais il faudra adoucir.
...
ggplot(foo3, aes(x=date2,y=tot))+
  ...
  theme_minimal()+theme(axis.text.x = element_text(size=11, angle=45))+
  labs(title="Intensité des émotions #confinementjour", subtitle="par heure", y="Somme des émotions",x=NULL)+  geom_smooth(method="gam")+
  scale_x_datetime(date_breaks = "1 day", labels = scales::label_date_short())
...
liwc<-readRDS(file="df_nrcliwc.rds")
...
```{r val4}
r_sent<-subset(liwc, select=c( negative,émonég,positive, émopos))
cor2 <- cor(r_sent)
corrplot.mixed(cor2)
...
```{r liwc3,}
M1<-subset(liwc, select=c(negative,anger,disgust, fear, sadness,surprise,anticipation,trust,joy,positive))
