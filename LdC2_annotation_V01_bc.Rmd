...
df<- readRDS(file = "DataPrimaires/df_consoj4244.rds") 
...
write_rds(df,"df_nrcliwc4244.rds")
...
df_nrcliwclsd4244<-cbind(df,dfBaseSpecifique)
write_rds(df_nrcliwclsd4244,"df_nrcliwclsd4244.rds")
M<-subset(df_nrcliwclsd4244, select=c(anger, anticipation,disgust, fear, joy, sadness, surprise,trust, negative, positive,émopos,émonég,Pos_lsdfr,Neg_lsdfr))
M <- cor(M)
library(corrplot)
corrplot.mixed(M, order="hclust")

df_nrcliwclsd<- readRDS(file = "df_nrcliwclsd.rds") 
df_nrcliwclsd4244<- readRDS(file = "df_nrcliwclsd4244.rds") 

df_nrcliwclsd_cum44 <-rbind(df_nrcliwclsd, df_nrcliwclsd4244)
