---
title: "Barometre du confinement"
author: "CB"
date: "31 mars 2020"
output:
...
L'approche ici est principalement baromètrique. On cherche à représenter l'évolution des indicateurs dans le temps d'abord à une échelle horaire, puis journalière. 
...
ggplot(foo, aes(x=date2,y=value,group=variable))+
 ...
 - La joie passe de la sérénité à l'extase
 \n \nSource: Data collected by Benavent C. from Twitter's REST API via rtweet")+
  scale_fill_manual(values=emocol)+  scale_x_datetime(date_breaks = "1 day", labels = scales::label_date_short())
...
foo<- df_nrc %>% group_by(month,day) %>% mutate(n=1) %>%   summarise(ami=mean(ami,na.rm=TRUE),famille=mean(famille,na.rm=TRUE),humain=mean(humain,na.rm=TRUE))
...
foo<-df_nrc %>% group_by(month,day) %>% mutate(n=1) %>% summarise(sentir=mean(sentir,na.rm=TRUE),voir=mean(voir,na.rm=TRUE),entendre=mean(entendre,na.rm=TRUE),mouvement=mean(corps,na.rm=TRUE),espace=mean(espace,na.rm=TRUE),temps=mean(temps,na.rm=TRUE))
foo$date<-paste0("2020","-",foo$month,"-",foo$day)
foo$date2 <- as.POSIXct(strptime(foo$date, "%Y-%m-%d"))
...
#l'organique
foo<- df_nrc%>% group_by(month,day) %>% mutate(n=1) %>% summarise(alimentation=mean(alimentation,na.rm=TRUE),sexualité=mean(sexualité,na.rm=TRUE),santé=mean(santé,na.rm=TRUE),corps=mean(corps,na.rm=TRUE))
foo$date<-paste0("2020","-",foo$month,"-",foo$day)
foo$date2 <- as.POSIXct(strptime(foo$date, "%Y-%m-%d"))
...
ggplot(data = cor3, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile()+ labs(title="corrélations entre les features et sentiments",x="feelings", y="topic")+
