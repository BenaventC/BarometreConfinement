...
happy_emojis <- df %>%
  ...
  unnest(emoji = c(emoji)) %>% #on crée une ligne par émoji
...
#####
foo<- df_nrcliwc %>% group_by(month,day) %>% 
 # mutate(n=1) %>%   
...
foo<-foo %>% select(date2,ami, famille, humain ) # ungroup %>% # eliminated
require(data.table); foo<-melt(foo,id=c("date2"))
...
# Les emojis (des tweets) les plus négatifs
emo_sent%>%
  group_by(text)%>%
...
# Les emojis (des tweets) les plus positifs
emo_sent%>%
  group_by(text)%>%
  summarise(sentiment=sum(sentiment_score))%>%
  ungroup()%>%
...
#Graphique de l'évolution du sentiment moyen (des emojis) dans le temps
